...
import { REFERENCE_VALUE_PREFIX, SORTED_KEY_PREFIX, SET_KEY_PREFIX, PLAIN_VALUE_PREFIX, LEAF_PREFIX, DEFAULT_DATABASE } from './consts'

export const hashUint8Array = (value: Uint8Array) => new Uint8Array(crypto.createHash('sha256').update(value).digest())

export const utf8Encode = (val: any): Uint8Array => new util.TextEncoder().encode(val)

export const digestKeyValue = (kv: schemaTypes.KeyValue) => {
    const key = kv.getKey_asU8()
    const value = kv.getValue_asU8()
    const valdigest = hashUint8Array(value)
...
export const utf8Decode = (val: any) => val === ''
    ? val
    : new util.TextDecoder("utf-8").decode(val);
    
export const base64Encode = (val: any) => btoa(String.fromCharCode(
    ...new Uint8Array(val))
)
...
export const maskConfig = (c: types.Config) => {
    ...
    if (c) {
        ...
        // c.database && (result += `, database: ${c.database || DEFAULT_DATABASE}`);
        ...
        c.rootPath && (result += `, rootPath: ${c.rootPath}`)
        c.certs && (result += `, certs: ${c.certs}`);
        c.apiKey && (result += `, apiKey: ${c.apiKey}`);
    }
    return `[ ${result} ]`;
}
    
export const maskString = (data: string, n = 12) => `${data.slice(0, n)}************${data.slice(data.length - n)}`
...
export const bitLength = (n: number): number => n === 0 ? n : Math.abs(n).toString(2).length